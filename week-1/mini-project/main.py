...
print(f"Extracted Role: {job.role}")
...
def final_score(job: JobD, resume: Resume) -> MatchResult:
    ...
    In the 'score' field, give the overall match percentage from 0 to 100 as a number.
...
all_results = []
files_to_process = [f for f in resume_folder.iterdir() if f.suffix.lower() in [".pdf", ".docx"]]

if not files_to_process:
    print(f"\nNo .pdf or .docx resume files found in '{resume_folder}'. Please add resume files there to process candidates.")
else:
